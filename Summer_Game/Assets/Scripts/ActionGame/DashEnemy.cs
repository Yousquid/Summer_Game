...
public class DashEnemy : MonoBehaviour
{
    ...
    public float dashDuration = 0.15f;  // 冲刺时间
    public float dashCooldown = 2f;     // 冲刺冷却时间

    private Vector2 dashDir;
    private float dashTimeRemaining;
    private bool isDashing = false;
    private float lastDashTime = -999;
    ...
    void Update()
    {
        ...
        switch (state)
        {
            ...
            case State.Charging:
                ...
                {
                    SoundSystem.instance.PlaySound("EnemyCharging2");
    ...
    void DoDashMovement()
    {
        ...
        // 本帧位移量
        Vector2 delta = dashDir * dashSpeed * Time.fixedDeltaTime;

        // 如果使用 StretchZone，则扭曲 delta
        if (stretchTracker != null && stretchTracker.currentZone != null)
        {
            delta = stretchTracker.currentZone.WarpDisplacement(rb.position, delta);
        }
